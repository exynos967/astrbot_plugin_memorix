"""Phase 1 依赖剥离桩：MaiBot ``src.*`` 中暂无对应插件 shim 的符号。
...
# MaiBot LLM 服务桩：llm_api.generate / llm_api.LLMServiceRequest / llm_api.LLMServiceResult
llm_api = _RecursiveStub()
LLMServiceResult = _RecursiveStub()
LLMServiceClient = _RecursiveStub()

# MaiBot 消息/会话桩：message_api.get_messages_by_time_in_chat /
